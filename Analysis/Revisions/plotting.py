...
def scatter_mds_2(xyz,task_id='a',fig_id=1,flipdims=False,items_per_dim=5,flipcols=False,marker_scale=1):
    ...
    if task_id == 'both':
        ...
        for ii in range(0,n_items//2):            
            plt.plot([x[ii],x[ii]],[y[ii],y[ii]],marker='s',markerfacecolor=ctxMarkerCol,markeredgecolor=ctxMarkerEdgeCol[0],markersize=ctxMarkerSize,markeredgewidth=.5*marker_scale)
            plt.plot(x[ii],y[ii],marker='h',markeredgecolor=scat_l[l[ii],:],markerfacecolor=scat_l[l[ii],:],markersize=scat_b[b[ii]])

        for ii in range(n_items//2,n_items):            
            plt.plot(x[ii],y[ii],marker='s',markerfacecolor=ctxMarkerCol,markeredgecolor=ctxMarkerEdgeCol[1],markersize=ctxMarkerSize,markeredgewidth=.5*marker_scale)
            plt.plot(x[ii],y[ii],marker='h',markeredgecolor=scat_l[l[ii],:],markerfacecolor=scat_l[l[ii],:],markersize=scat_b[b[ii]])
    else:
        for ii in range(0,n_items):
            
            plt.plot(x[ii],y[ii],marker='s',markerfacecolor=ctxMarkerCol,markeredgecolor=ctxMarkerEdgeCol,markersize=ctxMarkerSize,markeredgewidth=.5*marker_scale)
            plt.plot(x[ii],y[ii],marker='h',markeredgecolor=scat_l[l[ii],:],markerfacecolor=scat_l[l[ii],:],markersize=scat_b[b[ii]])
