function rsa_roi_parammod_fit(maskName)
    %% rsa_roi_parammod_fit()
    %
    ...
    comp_min = 0.05;
    comp_max = 0.9;
    comp_step = 0.05; % 10 percent steps to make this tractable
    ...
    betas_hat  = nan(length(params.num.goodSubjects),5);
    idces_hat  = nan(length(params.num.goodSubjects),5);
    for (ii = 1:length(params.num.goodSubjects))
        ...
        parfor(jj=1:size(idces,1),n_workers)        
            ...
            x_sub = nanzscore(vectorizeRDM(modRDM)');
            %% concatenate training subs                                 
            % get parameter estimates 
            b(jj) = regress(y_sub,x_sub);    
            % if mod(jj,1000)==0
            %     toc(t) 
            %     disp(['iteration ' num2str(jj) '/' num2str(size(idces,1))]);
            % end
        end        
  
        %% predict
        % find best-fitting parameters (training data)
        [~,m] = max(b,[],'all','linear');
        ...
        rdms(ii,:,:) = rdm_pred_f;
        betas_all(ii,:,:) = b;
        betas_hat(ii,:) = [comp_vect(idces(m,1)),comp_vect(idces(m,2)),comp_vect(idces(m,3)),comp_vect(idces(m,4)),rot_vect(idces(m,5))];
        idces_hat(ii,:) = idces(m,:);
        toc(t);
    end
    ...
    results.betas = betas_all;
    results.betas_hat = betas_hat;
    results.idces = idces;
    results.idces_hat = idces_hat;
    results.rdms = rdms;
  
    cd(params.dir.outDir);
    parsave(['results_regress_parametrised_rdms_' maskName],results);
