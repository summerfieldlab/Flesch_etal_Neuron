function results = rsa_searchlight_loso_sigtest()
  %% rsa_searchlight_loso_sigtest()
  %
  % leave one subject out statistical inference on beta maps 
  % 
  % Timo Flesch, 2020
  ...
  for subID = 1:length(params.num.goodSubjects)
      ...
      save(fullfile(params.dir.outDir, ['betas_searchlight_LOSO_' params.names.modelset '_sub_' num2str(subj) '_masked_STATS.mat']),'results');
  end
