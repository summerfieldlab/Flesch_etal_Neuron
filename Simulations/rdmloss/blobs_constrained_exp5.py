...
def main():
    ...
    rdm_constr1 = dmat[:,0][:,np.newaxis].T
    # parameters 
    N_EPISODES = 50000
    N_RUNS = 10
    N_HIDDEN = 100
    ...
    WEIGHT_LL = 1
    WEIGHT_RDM1 = 0.3 # was 0.05 for h1
    WEIGHT_RDM2 = 0.3 # was 0.05 for h1
    dmat.shape[0]
    ...
    y_rdm1     = tf.placeholder(tf.float32,[None,dmat.shape[0]],'model_rdm1')
    y_rdm2     = tf.placeholder(tf.float32,[None,dmat.shape[0]],'model_rdm2')
    ...
    y_h1_rdm = tf_compute_rdm(tf.transpose(y_h1))
    y_h1_flzs = tf_zscore(tf_compute_lowertriang_flattened(y_h1_rdm))
    ...
    loss_rdm1 = tf.reduce_sum(tf.pow(y_h1_flzs - y_rdm1,2),name='rdm_loss1')
    loss_rdm2 = tf.reduce_sum(tf.pow(y_h_flzs - y_rdm2,2),name='rdm_loss2')
    loss =  WEIGHT_LL*loss_supervised + WEIGHT_RDM1*loss_rdm1 + WEIGHT_RDM2*loss_rdm2 
    ...
    results = {
                ...
                'losses_rdm1' : np.empty((N_MODELS,N_RUNS,N_EPISODES)),
                'losses_rdm2' : np.empty((N_MODELS,N_RUNS,N_EPISODES)),
                ...
                'weight_ll': WEIGHT_LL,
                'weight_rdm1': WEIGHT_RDM1,            
                'weight_rdm2': WEIGHT_RDM2,            
                'n_hidden': N_HIDDEN,
    ...
    for ii in range(N_MODELS):
        ...
        rdm_constr1 = dmat[:,ii][:,np.newaxis].T # any for 1st
        rdm_constr2 = dmat[:,2][:,np.newaxis].T # parallel for 2nd
        ...
        for jj in range(N_RUNS):
            ...
            with tf.Session(config=config) as sess:
                ...
                for ep in range(N_EPISODES):
                    ...
                    # evaluation
                    rl1,rl2,sl,tl = sess.run([loss_rdm1,loss_rdm2,loss_supervised,loss], feed_dict={x_features:x_train,
                                                                        x_garden:c_train,
                                                                        y_reward:y_train,                                                            
                                                                        y_rdm1:rdm_constr1,
                                                                        y_rdm2:rdm_constr2})
                    
                    losses_rdm1.append(rl1)
                    losses_rdm2.append(rl2)
                    losses_supervised.append(sl)
                    losses_total.append(tl)
                    if (ep%1000)==0:
                        print('episode {}, interleaved, rdm1 loss {:.2f}, rdm2 loss {:.2f}, supervised loss {:.2f}, weighted total loss {:.2f}'.format(ep,rl1,rl2, sl, tl))
                ...
                results['losses_rdm1'][ii,jj,:] = losses_rdm1 
                results['losses_rdm2'][ii,jj,:] = losses_rdm2
    ...
    with open('final_blob_mlp_2h_rdm_both.pickle','wb') as f:
