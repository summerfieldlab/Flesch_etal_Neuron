...
def main():
    ...
    x_h = tf.add(tf.matmul(x_features,w_hf),tf.matmul(x_garden,w_hg)) + b_hf
    ...
    b_out = var_bias((1,1))
    # y_pred = tf.nn.sigmoid(tf.add(tf.matmul(y_h,w_out),b_out))
    ...
    varstoquery = [y_pred,w_hf,w_hg,w_out,y_h,x_h] 
    ...
    results = {
                ...
                'all_x_hidden' : np.empty((N_MODELS,N_RUNS,N_HIDDEN,x_train.shape[0])),
                'all_y_hidden' : np.empty((N_MODELS,N_RUNS,N_HIDDEN,x_train.shape[0])),
                'all_y_out' : np.empty((N_MODELS,N_RUNS,1,x_train.shape[0])),            
                'n_dead': np.empty((N_MODELS,N_RUNS,2)),
                'n_local': np.empty((N_MODELS,N_RUNS,2)),
                'n_only_a': np.empty((N_MODELS,N_RUNS,2)),
                'n_only_b': np.empty((N_MODELS,N_RUNS,2)),
                'hidden_dotprod' : np.empty((N_MODELS,N_RUNS,2)),
                'weight_ll': WEIGHT_LL,
    ...
    for ii in range(N_MODELS):
        ...
        for jj in range(N_RUNS):
            ...
            with tf.Session(config=config) as sess:
                ...
                # stats at init 
                responses,w_h1,w_h2,w_o,y_hidden,x_hidden = sess.run(varstoquery,feed_dict={x_features:x_in,x_garden:x_ctx})
                results['n_dead'][ii,jj,0],results['n_local'][ii,jj,0],results['n_only_a'][ii,jj,0],results['n_only_b'][ii,jj,0], results['hidden_dotprod'][ii,jj,0] = compute_sparsity_stats(y_hidden)
                ...
                # EVALUATION            
                responses,w_h1,w_h2,w_o,y_hidden,x_hidden = sess.run(varstoquery,feed_dict={x_features:x_in,x_garden:x_ctx})
                results['n_dead'][ii,jj,1],results['n_local'][ii,jj,1],results['n_only_a'][ii,jj,1],results['n_only_b'][ii,jj,1], results['hidden_dotprod'][ii,jj,1] = compute_sparsity_stats(y_hidden.T)
                ...
                results['all_y_hidden'][ii,jj,:] = y_hidden.T
                results['all_x_hidden'][ii,jj,:] = x_hidden.T
                results['all_y_out'][ii,jj,:] = responses.T
                sess.close()
                # tf.reset_default_graph()

    with open('final_blob_mlp_1h_baseline.pickle','wb') as f:
